...
source /opt/mhs/lib/menu/functions/serverid.sh
source /opt/mhs/lib/menu/functions/emergency.sh
source /opt/mhs/lib/menu/functions/serverid.sh
source /opt/mhs/lib/menu/functions/update.sh
#######################################################
...
if [[ "$wstatus" == "watchtower" ]]; then
	...
	ansible-playbook /opt/mhs/lib/menu/functions/ouroboros.yml
fi
ostatus=$(docker ps --format '{{.Names}}' | grep "ouroboros")
if [[ "$ostatus" != "ouroboros" ]]; then ansible-playbook /opt/mhs/lib/menu/functions/ouroboros.yml; fi
}
gcecheck() {
...
if [[ "$gcheck" == ".internal" ]]; then
        ...
        else bash /opt/mhs/lib/menu/pggce/gcechecker.sh; fi
...
rollingpart() {
  ...
  if [[ $(cat /var/plexguide/install.roles) != "$rolenumber" ]]; then
    ...
    ansible-playbook /opt/mhs/lib/menu/pgbox/community/community.yml
    ansible-playbook /opt/mhs/lib/menu/pgbox/core/core.yml
	clone
...
alias() {
ansible-playbook /opt/mhs/lib/menu/alias/alias.yml
}
aptupdate() {
ansible-playbook /opt/mhs/lib/menu/mhs.yml --tags update
}
cleaner() {
ansible-playbook /opt/mhs/lib/menu/mhs.yml --tags autodelete,clean,journal
}
dependency() {
  ospgversion=$(cat /var/plexguide/os.version)
  if [[ "$ospgversion" == "debian" ]]; then
    ansible-playbook /opt/mhs/lib/menu/dependency/dependencydeb.yml
  else
    ansible-playbook /opt/mhs/lib/menu/dependency/dependency.yml
  fi
}
docstart() {
ansible-playbook /opt/mhs/lib/menu/mhs.yml --tags docstart
}
folders() {
ansible-playbook /opt/mhs/lib/menu/installer/folders.yml
}
prune() {
ansible-playbook /opt/mhs/lib/menu/prune/main.yml
}
gcloud() {
ansible-playbook /opt/mhs/lib/menu/mhs.yml --tags gcloud_sdk
}
mergerfsinstall() {
ansible-playbook /opt/mhs/lib/menu/mhs.yml --tags mergerfsinstall
}
motd() {
ansible-playbook /opt/mhs/lib/menu/motd/motd.yml
}
mountcheck() {
ansible-playbook /opt/mhs/lib/menu/installer/mcdeploy.yml
}
newinstall() {
...
pgdeploy() {
touch ${abc}/pg.edition && bash /opt/mhs/lib/menu/start/start.sh
}
pgedition() {
...
portainer() {
  ...
  if [[ "$dstatus" != "portainer" ]]; then ansible-playbook /opt/mhs/lib/menu/functions/portainer.yml; fi
}
# Roles Ensure that PG Replicates and has once if missing; important for startup, cron and etc
pgcore() {
file="${abc}/new.install"
if [[ -f "$file" ]]; then ansible-playbook /opt/mhs/lib/menu/pgbox/core/core.yml; fi
if [[ -f "$file" ]]; then ansible-playbook /opt/mhs/lib/menu/pgbox/community/community.yml; fi
}
clone() {
file="${abc}/new.install"
if [[ -f "$file" ]]; then
     ...
     ansible-playbook "/opt/mhs/lib/menu/pgcloner/clone/primary.yml"
	 sleep 0.1
...
dockerinstall() {
file="${abc}/new.install"
if [[ -f "$file" ]]; then ansible-playbook /opt/mhs/lib/menu/mhs.yml --tags docker; fi
}
####EOF###
