...
EOF
  ...
  Y) ansible-playbook /opt/mhs/lib/menu/mhs.yml --tags mergerfsupdate ;;
  y) ansible-playbook /opt/mhs/lib/menu/mhs.yml --tags mergerfsupdate ;;
  z) clear && exit ;;
  Z) clear && exit ;;
  *) badinput ;;
...
rcloneupdate(){
check=$(bash /opt/mhs/lib/menu/pgui/templates/check.sh >/dev/null 2>&1)
...
EOF
  ...
  case $typed in
  Y) ansible-playbook /opt/mhs/lib/menu/mhs.yml --tags rcloneinstall && $check ;;
  y) ansible-playbook /opt/mhs/lib/menu/mhs.yml --tags rcloneinstall && $check ;;
  z) clear && exit ;;
  Z) clear && exit ;;
  *) badinput ;;
