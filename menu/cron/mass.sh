...
weekrandom() {
  while read p; do
    ...
    ansible-playbook /opt/mhs/lib/menu/cron/cron.yml
...
manualuser() {
  while read p; do
    echo "$p" >/tmp/program_var
    bash /opt/mhs/lib/menu/cron/cron.sh
  done </var/plexguide/pgbox.buildup
  exit
}
...
EOF
  ...
  if [ "$typed" == "1" ]; then
    ...
    manualuser && ansible-playbook /opt/mhs/lib/menu/cron/cron.yml
  elif [ "$typed" == "3" ]; then
    dailyrandom && ansible-playbook /opt/mhs/lib/menu/cron/cron.yml
  elif [ "$typed" == "4" ]; then
    weekrandom && ansible-playbook /opt/mhs/lib/menu/cron/cron.yml
  else badinput1; fi
}
