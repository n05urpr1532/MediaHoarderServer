...
value() {
bash /opt/mhs/lib/menu/pgbox/value.sh
}
...
cronexe() {
  ...
  if [ "$croncheck" == "0" ]; then bash /opt/mhs/lib/menu/cron/cron.sh; fi
}
...
initial() {
  ...
  folder && ansible-playbook /opt/mhs/lib/menu/pgbox/community/community.yml >/dev/null 2>&1
...
question2() {
  ...
  if [[ "$edition" == "PG Edition - HD Solo" ]]; then
    ...
    if [ "$croncount" -ge 2 ]; then bash /opt/mhs/lib/menu/cron/mass.sh; fi
  fi

  # CName & Port Execution
  bash /opt/mhs/lib/menu/pgbox/cname.sh
...
EOF
    ...
    # End Banner
    bash /opt/mhs/lib/menu/pgbox/endbanner.sh >>/tmp/output.info
