...
templatebackup=/opt/mhs/lib/menu/pgscan/templates/config.backup
template=/opt/mhs/lib/menu/pgscan/templates/config.json.j2
X_PLEX_TOKEN=$(sudo cat "/opt/appdata/plex/database/Library/Application Support/Plex Media Server/Preferences.xml" | sed -e 's;^.* PlexOnlineToken=";;' | sed -e 's;".*$;;' | tail -1)
...
pasdeploy() {
ansible-playbook /opt/mhs/lib/menu/pgscan/yml/plexautoscan.yml
clear
}
undeployed() {
