...
clonestartoutput() {
  ...
  echo ""
  if [[ "$demo" == "ON " ]]; then mainid="********"; else mainid="$uploaderemail"; fi
  if [[ "$transport" == "mu" ]]; then
    tee <<- EOF
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

[1] Client ID & Secret       [ ${uploaderid} ]
[2] GDrive                   [ $gstatus ]
...
[2] Project Name             [ $uploaderproject ]
[3] Client ID & Secret       [ ${uploaderid} ]
...
localstartoutput() {
  case $typed in
    1) executelocal ;;
    2) bash /opt/mhs/lib/core/multihd/multihd.sh ;;
    3) transportselect ;;
    z) exit ;;
    Z) exit ;;
    *) clonestart ;;
...
clonestartactions() {
  if [[ "$transport" == "mu" ]]; then
    case $typed in
      1) keyinputpublic ;;
      2) publicsecretchecker && echo "gdrive" > /var/mhs/state/rclone/deploy.version && oauth ;;
      z) exit ;;
      Z) exit ;;
    ...
    case $typed in
      ...
      3) publicsecretchecker && passwordcheck && echo "gdrive" > /var/mhs/state/rclone/deploy.version && oauth ;;
      z) exit ;;
      Z) exit ;;
    ...
    case $typed in
      ...
      5) publicsecretchecker && tlabelchecker && echo "tdrive" > /var/mhs/state/rclone/deploy.version && oauth ;;
      6) publicsecretchecker && echo "gdrive" > /var/mhs/state/rclone/deploy.version && oauth ;;
    ...
    case $typed in
      ...
      6) publicsecretchecker && passwordcheck && tlabelchecker && echo "tdrive" > /var/mhs/state/rclone/deploy.version && oauth ;;
      7) publicsecretchecker && passwordcheck && echo "gdrive" > /var/mhs/state/rclone/deploy.version && oauth ;;
...
demomode() {
  if [[ "$demo" == "OFF" ]]; then
    echo "ON " > /var/mhs/state/uploader.demo
  else echo "OFF" > /var/mhs/state/uploader.demo; fi
