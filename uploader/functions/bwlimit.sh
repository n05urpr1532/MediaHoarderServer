...
EOF
  ...
  if [[ "$typed" == "Z" || "$typed" == "z" ]]; then
    ...
    if [[ "$typed" -ge "0" && "$typed" -le "10000" ]]; then
      echo "${typed}M" > "/var/mhs/state/$bwlimitVar"
...
addCustomTimeTablePart() {
  timetable=$(cat /var/mhs/state/timetable.bw)
  tee <<- EOF
...
EOF
  ...
  if [[ "$typed" == "z" || "$typed" == "Z" ]]; then
    ...
    if [[ "$typed" == "a" || "$typed" == "A" ]]; then
      if [[ $(cat /var/mhs/state/timetable.bw) != "" ]]; then
        echo -n "$(cat /var/mhs/state/timetable.bw)" > /var/mhs/state/$bwlimitVar
      else
        badinput
        setCustomTimeTable
      fi
    ...
    then
      ...
      echo -n "" > /var/mhs/state/timetable.bw
      setCustomTimeTable
    fi
  fi
}
