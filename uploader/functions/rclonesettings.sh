...
# bwlimit=$(cat "/var/mhs/state/$bwlimitVar")
...
# if [[ "$menuSelection" == "5" ]]; then echo "${typed}${sizeSuffix}" >/var/mhs/state/vfs_bs; fi
# if [[ "$menuSelection" == "2" ]]; then echo "${typed}${sizeSuffix}" >/var/mhs/state/vfs_dcs; fi

# if [[ "$menuSelection" == "11" ]]; then echo "${typed}${sizeSuffix}" >/var/mhs/state/vfs_dct; fi
# if [[ "$menuSelection" == "6" ]]; then echo "${typed}${sizeSuffix}" >/var/mhs/state/vfs_rcs; fi
# if [[ "$menuSelection" == "9" ]]; then echo "${typed}${sizeSuffix}" >/var/mhs/state/vfs_cma; fi

# if [[ "$menuSelection" == "3" ]]; then
# echo "${typed}" >/var/mhs/state/vfs_t
# echo "$((${typed} * 2))" >/var/mhs/state/vfs_c
# fi
# if [[ "$menuSelection" == "4" ]]; then echo "${typed}${sizeSuffix}" >/var/mhs/state/vfs_mt; fi

# if [[ "$menuSelection" == "7" ]]; then
# if [[ "$typed" == "0" ]]; then
# echo "off" >/var/mhs/state/vfs_rcsl
# else
# echo "${typed}${sizeSuffix}" >/var/mhs/state/vfs_rcsl
# fi
# fi

# if [[ "$menuSelection" == "8" ]]; then
# if [[ "$typed" == "1" ]]; then echo "off" >/var/mhs/state/vfs_cm; fi
# if [[ "$typed" == "2" ]]; then echo "minimal" >/var/mhs/state/vfs_cm; fi
# if [[ "$typed" == "3" ]]; then echo "writes" >/var/mhs/state/vfs_cm; fi
# if [[ "$typed" == "4" ]]; then echo "full" >/var/mhs/state/vfs_cm; fi
# fi

# if [[ "$menuSelection" == "10" ]]; then
# if [[ "$typed" == "0" ]]; then
# echo "off" >/var/mhs/state/vfs_cms
# else
# echo "${typed}${sizeSuffix}" >/var/mhs/state/vfs_cms
# fi
# fi

# if [[ "$menuSelection" == "12" ]]; then
# if [[ "$typed" == "1" ]]; then echo "DEBUG" >/var/mhs/state/vfs_ll; fi
# if [[ "$typed" == "2" ]]; then echo "INFO" >/var/mhs/state/vfs_ll; fi
# if [[ "$typed" == "3" ]]; then echo "NOTICE" >/var/mhs/state/vfs_ll; fi
# if [[ "$typed" == "4" ]]; then echo "ERROR" >/var/mhs/state/vfs_ll; fi
# fi
...
# echo $(sed -e 's/^"//' -e 's/"$//' <<<$(cat /var/mhs/state/uagent)) >/var/mhs/state/uagent; fi
...
# rclone --config /opt/mhs/etc/rclone/rclone.conf \
# --stats 1s -P \
# --tpslimit=10 \
# --checkers="$vfs_c" \
# --transfers="$vfs_t" \
...
# invalidInputNotice
# else
# echo "${test_size}G" >/var/mhs/state/vfs_test
