...
readrcloneconfig() {
  ...
  tccheck=$(cat /opt/mhs/etc/rclone/rclone.conf | grep "remote = tdrive:/encrypt")
  if [ "$tccheck" != "" ]; then
    echo "good" > /var/mhs/state/rclone/tcrypt.status && tcstatus="good"
  else echo "bad" > /var/mhs/state/rclone/tcrypt.status && tcstatus="bad"; fi
