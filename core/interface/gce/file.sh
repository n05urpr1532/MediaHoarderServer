...
if [ "$menu" == "4" ]; then
  ...
  gcloud projects list && gcloud projects list > /var/mhs/state/projects.list
  echo ""
  echo "------------------------------------------------------------------------------"
  ...
  echo $typed > /var/mhs/state/project.final
  echo 'INFO - Selected: Exiting Application Suite Interface' > /var/mhs/log/pg.log && bash /opt/mhs/lib/core/log/log.sh
  exit
fi
...
if [ "$menu" == "5" ]; then
  ...
  ### Part 1
  pcount=$(cat /var/mhs/state/project.processor)
  echo ""
  echo "--------------------------------------------------------"
  ...
  while [ "$break" == "off" ]; do
    ...
    if [ "$tcheck" == "" ]; then
      ...
      echo "----------------------------------------------"
      echo ""
      echo $typed > /var/mhs/state/project.processor
      read -n 1 -s -r -p "Press [ANY KEY] to Continue "
      break=on
    fi
  done
...
if [ "$menu" == "6" ]; then
  ...
  ### Part 2
  #gcloud compute regions list | awk '{print $1}' | tail -n +2 > /var/mhs/state/project.region
  ...
  echo "NOTE: Please Standby"
  echo ""
  projectname=$(cat /var/mhs/state/project.final)
  region=$(cat /var/mhs/state/project.ipregion)
  gcloud compute addresses create pg-gce --region $region --project $projectname
  gcloud compute addresses list | grep pg-gce | awk '{print $3}' > /var/mhs/state/project.ipaddress
  ...
  while [ "$break" == "off" ]; do
    ...
    if [ "$tcheck" == "" ]; then
      ...
      echo "----------------------------------------------"
      echo ""
      echo $typed > /var/mhs/state/project.location
      read -n 1 -s -r -p "Press [ANY KEY] to Continue "
      break=on
    fi
  done
...
if [ "$menu" == "7" ]; then
  ...
  project=$(cat /var/mhs/state/project.final)
  ipaddress=$(cat /var/mhs/state/project.ipaddress)
  location=$(cat /var/mhs/state/project.location)
  region=$(cat /var/mhs/state/project.ipregion)
  cpu=$(cat /var/mhs/state/project.processor)
  ...
  inslist=$(gcloud compute firewall-rules list | grep mhs)
  if [ "$inslist" == "" ]; then
    echo "--------------------------------------------------------"
  ...
  if [ "finalchecks" != "" ]; then
    ...
    echo ""
    read -n 1 -s -r -p "Press [ANY KEY] to Continue "
    touch /var/mhs/state/gce.deployed
  else
    echo "--------------------------------------------------------"
...
if [ "$menu" == "8" ]; then
  ...
  echo ""
  echo ""
  ipproject=$(cat /var/mhs/state/project.location)
...
if [ "$menu" == "9" ]; then
  ...
  rm -rf /var/mhs/state/gce.deployed 1> /dev/null 2>&1
  echo ""
  echo "--------------------------------------------------------"
