...
if [ "$menu" == "2" ]; then
  ...
  while [ "$break" == "no" ]; do
    ...
    if [[ ! $REPLY =~ ^[Yy]$ ]]; then
      ...
      echo ""
      echo $typed > /var/mhs/state/server.id
      break=yes
      read -n 1 -s -r -p "Press [ANY KEY] to Continue "
      echo ""
    fi
...
idtest=$(cat /var/mhs/state/server.id)
if [ "$idtest" == "NOT-SET" ]; then
  ...
  echo
  read -n 1 -s -r -p "Press [ANY KEY] to Continue "
  echo serverid > /var/mhs/state/type.choice && bash /opt/mhs/lib/core/core/scripts/main.sh
  exit
fi
