...
if grep -q "gdrive:" $config; then
  ...
  --timeout=30m \
  --exclude="**encrypt**" \
  --config /opt/mhs/etc/rclone/rclone.conf | awk '{print $3,$4}' > $log/gduncrypt.log
...
if grep -q "tdrive:" $config; then
  ...
  --timeout=30m \
  --exclude="**encrypt**" \
  --config /opt/mhs/etc/rclone/rclone.conf | awk '{print $3,$4}' > $log/tduncrypt.log
...
if grep -q "gcrypt:" $config; then
  ...
  --no-update-modtime \
  --timeout=30m \
  --config /opt/mhs/etc/rclone/rclone.conf | awk '{print $3,$4}' > $log/gdcrypt.log
...
if grep -q "tcrypt:" $config; then
  ...
  --no-update-modtime \
  --timeout=30m \
  --config /opt/mhs/etc/rclone/rclone.conf | awk '{print $3,$4}' > $log/tdcrypt.log
