...
file3="$(tail -n 1 /var/mhs/state/gce.done)"
file3b="/var/mhs/state/gce.done"
...
if [[ "$file3" == "1" ]]; then
  ...
  if [ "$gcheck" == ".internal" ]; then
    ...
    #Check for NVME
    lsblk | grep nvme | awk '{print $1}' > /var/mhs/state/nvme.log
    lsblk | grep nvme | awk '{print $1}' > /var/mhs/state/nvmeraid.log
    sed -i 's/nvme0n//g' /var/mhs/state/nvmeraid.log
    #Check for NVME
    nvme="$(tail -n1 /var/mhs/state/nvmeraid.log)"
    ...
    if [[ "$nvme" == "2" ]]; then
      ...
      echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
      echo -e "1" > /var/mhs/state/gce.done
      sleep 2
    elif [[ "$nvme" == "3" ]]; then
    ...
    rm -rf /var/mhs/state/nvmeraid.log 1> /dev/null 2>&1

    echo "feeder" > /var/mhs/state/pg.server.deploy
    cat /var/mhs/state/pg.edition > /var/mhs/state/pg.edition.stored
    ...
    rm -rf /var/mhs/state/gce.false 1> /dev/null 2>&1
    rm -rf /var/mhs/state/nvme.log 1> /dev/null 2>&1
    rm -rf /var/mhs/state/nvmeraid.log > /dev/null 2>&1
  else
    touch /var/mhs/state/gce.false
  fi
else
